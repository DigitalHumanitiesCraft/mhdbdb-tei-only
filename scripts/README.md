...
- `--works --check` ist das CI-Gate (#399, `data-integrity.yml`): vergleicht Inhalte statt Formatierung, schreibt nichts, Exit 1 bei Abweichung, 0,6 s über alle 667 Dateien. Rot wird es auch, wenn eine Datei gar keinen lesbaren `msIdentifier` mehr hat (fehlender Block oder nicht parsebares XML). Das war bis zur Bot-Runde vom 07.09. nur eine Warnung, und damit konnte der ganze Block verschwinden, ohne dass das Gate anschlug.
