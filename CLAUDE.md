...
- **On a data PR the CI review bot goes red, and the local review therefore has to carry the PR. But open the bot's comment before dismissing it: a red check is not a finding, and it is not proof that there is none.** Four runs, each read in the job log rather than inferred from the check. **#368** (531 files, one of them a 40 MB binary): the platform computed no diff, and the bot aborted with `error_max_turns` after 6:24 min without ticking a single item of its own checklist. **#379**: the diff *was* computed (16 entries in the prompt) and the run still ended in a genuine `error_max_turns` (`is_error: true`) having delivered nothing. **#377** (34 entries) and **#382** (10 entries): the bot ticked its checklist in full, delivered two findings and four respectively, and reported `subtype: success` with `is_error: false`. Those two were **finished reviews that the action merely marked red**, because they had spent 59 and 56 turns against a `--max-turns` of 50. Nothing is lost when that happens: the comment is posted about ninety seconds into the run (09:10:53 on a run that failed at 09:20:37, 13:10:42 on one that failed at 13:19:28) and it survives the failure. Only the green tick is discarded, never the review. That turn number is ours, not a limit of the bot, and it has been 100 since `6fbf7e002`. There is no way to make the action tolerate it: `run-claude-sdk.ts` in the pinned `v1` throws unconditionally on `success` above the budget, and no input controls that, so do not go looking for one. So a computed diff is necessary but not sufficient, and what a given run will do is not predictable from outside. #377 is the expensive case and the reason this bullet is worded as an instruction: its comment carried two measured findings, the red check was dismissed as a platform limit nine minutes later, and the PR was merged with the comment unread. **Do not use the REST field to judge whether the diff was computed:** `gh api .../pulls/377 -q .changed_files` returns 0 while `gh pr view 377 --json files` returns 34, and that false 0 is exactly what the #377 dismissal was built on. Read the `<changed_files>` block in the job log, or use `--json files`. A run that genuinely aborted aborts identically on a rerun, so do not start one: comment the situation on the PR instead, or the red check gets read as a finding. And do not split the PR into a corpus commit and an index commit just to get the bot to engage. That buys a bot pass with a state in which the index does not match the corpus, which is exactly what the Data-Change-Lifecycle forbids. What actually matters is checked anyway: `data-integrity.yml` rebuilds `variants.xml`, both indexes and the static API and compares them against the committed state, which is a stronger statement about a data PR than any reading of the diff.
  - **A limit that holds regardless of whether the run completes: the bot cannot execute Python.** It says so in its own comment, on #386 on 2026-09-02: „Python ist in diesem Job nicht ausführbar (`python3 …` wird von der Berechtigungsschicht abgelehnt). Ich konnte das Skript also nicht laufen lassen." It listed the numbers it could not check (227.652, 7.546.243, 210.262, the 430, 92,4 %), declared them neither confirmed nor disputed, and used `grep` for what was checkable without a run. **On a measurement PR the bot therefore reviews the code and not the result.** Relying on it for the numbers means relying on something that is not there: that is what the local `fable-reviewer` is for, which has Bash and can recompute. Not a defect of the bot, and the reason the local round stands beside it rather than instead of it. It does this well when it does it: on #391 it recomputed fifteen numbers from a prose entry with command and result, and flagged the three it could not measure as unchecked instead of passing over them.
  - **Four green runs since 2026-08-31, and no all-clear follows from them.** Measured in the job logs: #377 (34 files) 59 turns red, #382 (13) 56 red, #386 (9) 28 then 27 green, #388 (5) 28 green, #389 (18) 46 green, #391 (1) green. The obvious explanation, the raise of `--max-turns` to 100 in `6fbf7e002`, does **not** hold: every green run stayed below the old budget of 50 and would have been green without it. Neither does diff size: 18 files took fewer turns than 13. **What aborted #368 and #379 is still open**, and the plausible quantities have been ruled out one by one, so do not adopt a fresh explanation without measuring it. Two reading rules from the same measurements: a merge marks the then-running bot `skipped` and two rapid pushes produce `cancelled`, **neither of which is a statement about the review**, and `gh pr checks` shows only the last run (on #386 it says `SKIPPED` although two complete reviews exist). Use `gh run list --workflow claude-code-review.yml` for the history.
...
- **`auto:*`** is the one that governs a session's behaviour: `auto:full` (just do it) | `auto:brief` (1 to 3 questions up front, then run to completion) | `auto:checkin` (decision points along the way: sampling, batch size, scope) | `auto:pair` (only in a session with Chris) | `auto:blocked` (waiting on KZW, Julia, Linda, or an external party such as Carina, Alan, Silvan or a funder). The dividing line between the last two: a missing **answer from a third party** is `blocked`, a need for **Chris's time and authority** is `pair`.
