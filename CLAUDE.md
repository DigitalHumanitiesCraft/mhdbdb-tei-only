...
- **On a data PR the CI review bot goes red, and the local review therefore has to carry the PR. But read the bot's comment before dismissing it.** The red check is reliable; an empty one is not. On #368 (531 files, one of them a 40 MB binary) it aborted with `error_max_turns` after 6:24 min without ticking a single item of its own checklist, and the GitHub API reported `changedFiles: 0` for the PR, so the platform had not computed the diff either. On #382 (10 files, one of them a 26 MB corpus file) the platform *did* compute the diff, the prompt carried all ten entries, and the bot ticked its full checklist and produced four findings before the run died at 56 of 50 turns; all four were adopted after re-measurement, including one the local reviewer had missed. So the red check is reliable, but what varies is whether the platform computed the diff, and that is what decides whether the comment carries anything. Read it before dismissing it. A rerun aborts identically, so do not start one: comment the situation on the PR instead, or the red check gets read as a finding. And do not split the PR into a corpus commit and an index commit just to get the bot to engage. That buys a bot pass with a state in which the index does not match the corpus, which is exactly what the Data-Change-Lifecycle forbids. What actually matters is checked anyway: `data-integrity.yml` rebuilds `variants.xml`, both indexes and the static API and compares them against the committed state, which is a stronger statement about a data PR than any reading of the diff.
