 ...
 * short lemma hiding anywhere inside the input. Searching "böses" (normalized
 * `boeses`) returned ês/es, ô/o and sê/se next to the one right answer bœse/boese,
 * because the lexicon holds 5 one-letter, 98 two-letter and 598 three-letter
 * normalized forms. Middle High German inflection is suffixal, so a prefix test
 * keeps the useful cases (stem input → full lemma, inflected input → lemma) and
 * drops the noise. Measured over 300 seeded variant forms
 * (scripts/audit/measure-stage3-resolution.py): top-1 accuracy 0.3 % → 10.0 %,
 * median result list 8 → 0, largest list 108 → 8, recall 11.3 % → 10.7 %.
 */
