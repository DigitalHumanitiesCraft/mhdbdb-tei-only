...
- `<w>` - Word tokens with `@lemmaRef` and `@wordRef` attributes
...
These vocabularies ensure consistency and enable sophisticated cross-referencing.
...
- Large initial download (~24 MB indexes)
