...
| `sync/extract-variants.py` | tei/*.tei.xml | `tei:w` with `@lemmaRef` + `@corresp`, via `iter()` over the **whole document** | Regenerates `variants.xml` from the corpus. Note the scope: `build-corpus-index.py` restricts itself to `<body>`, this one does not. Identical over the current data (all 9,431,311 `<w>` sit in `<body>`) |
