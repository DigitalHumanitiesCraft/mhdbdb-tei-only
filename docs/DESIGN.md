...
Twelve analysis modules under `playground/js/ui/tei/` share a consistent structure (the router `tei-ui.js`, the modal controller `multi-lemma-search.js` and the shared scope helper `corpus-scope.js` do not follow the pattern). Anyone adding a new analysis tool should follow it: modules that deviate for no visible reason make the playground feel inconsistent and break conventions the router and the sidebar buttons build on.
