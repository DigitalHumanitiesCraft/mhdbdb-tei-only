...
32. **Ein grünes Em-Dash-Gate belegt nichts für `docs/`.** `scripts/audit/check-no-em-dash.py` prüft die ausgelieferten HTML-Seiten, `assets/js/`, `playground/`, `lemma/` und `assets/css/`. `docs/`, `publications/`, `tei/`, `authority-files/`, `schema/` und `testing/` sind ausdrücklich **nicht** im Umfang; es steht im Docstring des Skripts und ist an `GLOBS` ablesbar. Die Schreibregel gilt dort trotzdem. Wer Prosa in `docs/` schreibt, prüft sie von Hand, und zwar auf vier Schreibweisen: das Zeichen U+2014 selbst, die benannte mdash-Entity und die numerischen Entities dezimal 8212 und hexadezimal 2014 und schreibt „Gate grün" nicht als Beleg in den PR, denn das Gate hat die Datei nie gesehen. Gegenprobe am 2026-07-31: 8 Markdown-Dateien unter `docs/` enthalten Em-Dashes, ohne dass das Gate je rot war (dazu zwei Linecode-CSVs unter `docs/data/`, dort ist der Strich Datenzeichen und keine Typografie).
