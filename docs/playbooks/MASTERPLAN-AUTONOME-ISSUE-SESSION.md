...
28. **Vor dem ersten Commit die Branch-Basis messen, nicht den Branchnamen lesen.** Die Frage ist nicht „auf welchem Branch stehe ich", sondern „wie weit hängt der hinter `origin/main`, und liegt sein Inhalt oben schon". Am 2026-07-30 wurde #248 auf `feature/236-frauenlob` committet: 10 Commits zurück, Inhalt seit PR #253 gemergt, Remote-Branch gelöscht. Als PR hätte das bereits Gemergtes erneut vorgeschlagen und die Nachbesserungen aus `9521d27b6` überschrieben; die Korrektur kostete einen vollständigen Neuaufbau. Der Fehler ist **still**: `git status` meldet „working tree clean". Zwei Zeilen genügen, `git fetch --quiet origin` und `git rev-list --count HEAD..origin/main`; als Gate steht das in [`scripts/audit/check-branch-base.py`](../../scripts/audit/check-branch-base.py).
29. **Jede Session in ihrem eigenen Worktree, und am Ende räumt sie ihn wieder ab.** Der Hauptordner bleibt Referenz auf `main` und wird nicht als Arbeitsplatz benutzt; nur so sehen parallele Sessions einander nicht und keine verschiebt der anderen den Stand. Nach dem Merge gehören Branch **und** Worktree weg, im selben Zug, nicht in einer Aufräumsitzung Wochen später: am 2026-07-30 lagen vier Worktrees und acht Branches herum, drei davon aus Sessions vom 9. Juli, dazu vier verwaiste Verwaltungsordner in `.git/worktrees`. Vor dem Löschen den Merge belegen (`gh api "repos/<R>/pulls?state=all&head=<Org>:<branch>"`), aktive Worktrees an den Dateizeiten erkennen und in Ruhe lassen.
