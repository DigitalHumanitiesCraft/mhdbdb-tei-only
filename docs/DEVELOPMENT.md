...
8. **Naming-Index Konsistenz** (#152) — `source.commit`-Provenienz vorhanden + alle `works[].sigle` existieren als `tei/<SIG>.tei.xml` (ein Sigle-Rename bräche den Reader-Link im Playground sonst still). Offline, läuft immer.
...
10. **Cross-Reference-Integrity** (#44/#115/#152) — dangling Refs außerhalb `lexicon.xml` brechen den Build; `lexicon.xml` wird gegen die gepinnte Backfill-Baseline (977 Refs / 349 IDs, Konstanten in `check-authority-cross-refs.py`) gegated: Wachstum = rot, tolerierter Altbestand = grün, Ratsche bei Backfill senken.
...
| `check-authority-cross-refs.py` | **Korpus→Authority** Cross-Ref-Integrität: dangling `@lemmaRef`/`@ana`/`@corresp`/`@ref`/`@target`. `--check` = CI-Gate in `data-integrity.yml`: unresolved refs außerhalb `lexicon.xml` = sofort rot; `lexicon.xml` gegen gepinnte Backfill-Baseline (`LEXICON_BASELINE_*`-Konstanten, #152) — Wachstum rot, Altbestand grün. Einziger Detektor der Derived-File-Drift (#44/#115) |
