...
| `check-no-em-dash.py` | Gate in `no-cdn-check.yml` (#140): no em-dashes in user-visible text, comments exempt. HTML, JS and CSS are checked in full; Markdown only where a reader ends up (`publications/`, `README.md`, plus `schema/README.md` and `ingest/ari/README.md`, which the help pages link to), and there only in the lines that are new against `--diff-base <rev>` (#292, scope narrowed 2026-08-03). Both conditions must hold, and they are independent: files written purely for LLMs (`docs/`, `CLAUDE.md`, the JOURNAL) carry no constraints at all, so their Markdown is never scanned, while existing prose in a blog draft stays untouched. A green run therefore says nothing about a docs-only PR; the gate prints its own scope. `--selftest` secures scanner, file selection, scope and diff layer against mutations, because several earlier versions were fail-opens that passed every test case |
