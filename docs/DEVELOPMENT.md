...
| `doc-count-audit.py` | Drift-Detektor zwischen tatsächlichen Korpus-/Authority-Zahlen und den in der Doku verankerten Werten. Heuristik: Window ±2 absolut oder ±2 % relativ, strikter Keyword-Anchor unmittelbar nach der Zahl. Läuft als Health-Check-Werkzeug von Hand, kein Workflow ruft es. Dazu eine Selbstprüfung, die meldet, welche konfigurierten (Datei, Key)-Paare nichts prüfen: entweder fehlt das Ankerwort, oder es steht nur in Prosa statt hinter einer Zahl (#342) |
...
| `check-no-em-dash.py` | Gate in `no-cdn-check.yml` (#140): keine Em-Dashes in user-sichtbarem Text, Kommentare ausgenommen. HTML, JS und CSS werden ganz geprüft; Markdown nur in den Zeilen, die gegenüber `--diff-base <rev>` neu sind (#292), ohne Fences und Inline-Code. `--selftest` sichert Scanner, Dateiauswahl und Diff-Schicht gegen Mutationen ab, weil mehrere frühere Fassungen Fail-opens waren, die alle Testfälle bestanden |
