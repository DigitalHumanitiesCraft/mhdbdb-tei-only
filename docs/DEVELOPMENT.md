...
The same run also gates the directory tree in [`scripts/README.md`](../scripts/README.md), which lists the same scripts a second time and one directory level wider (the `scripts/` root plus `audit/`, `sync/` and `_archived/`). It was added on 2026-08-06 after `check-file-sizes.py` turned up here and in `file-size-check.yml` but not in that tree: two inventories of the same thing, one gated and one not, and the ungated one wins because it sits next to the code and gets read first.
...
| `check-doc-inventories.py` | Inventory gate (#329) for three listings, in both directions each: „Test File Inventory" in this file against `testing/tests/`, „Audit Scripts Reference" in this file against `scripts/audit/`, and the directory tree in `scripts/README.md` against `scripts/` plus `audit/`, `sync/`, `_archived/` (added 2026-08-06). Tables are read outside code blocks, the tree is read as one, which is the same rule from both sides: what is the listing here is the example there. stdlib only, called by `no-cdn-check.yml`; `--selftest` checks the scanner against synthetic input |
