...
- Set up modular UI components (25 modules; 24 until #204 added `corpus-scope.js`, 25 before #314 removed `file-display.js` and `progress.js`, 23 until #193 added `horses-explorer.js`)
...
│   ├── genre-explorer.js
│   └── name-explorer.js
├── tei/               # TEI text analysis (15 files: router + modal + scope helper + 12 analysis modules)
│   ├── tei-ui.js                  # Router/entry point, not an analysis tool
│   ├── corpus-scope.js            # Shared wording for the empty selection (#204), no entry point of its own
...
**Playground TEI module convention:** `playground/js/ui/tei/` holds fifteen files. The twelve analysis modules share one constructor/`show()`/`render()` pattern, with thunks instead of direct data references, a state-driven `renderBody()`, per-module escape helpers, and a MessageChannel yield for large aggregations. Three files are exempt: `tei-ui.js` is the router, `multi-lemma-search.js` is a documented modal outlier, and `corpus-scope.js` (#204) is a shared helper with no view and no entry point of its own. Eight of the modules import from it, but not for the same thing: seven take `emptyScopeMessage` for the wording of an empty selection, while `rhyme-dictionary.js` takes only `singleSelectedTextId`, which exists for the tools that carry their own text filter and therefore must not narrow the selection silently. Two are a special case of their own: `naming-explorer.js` (#59) and `horses-explorer.js` (#193) do not read the corpus index but lazily fetch their own small index (`data/naming-index.json.gz`, `data/horses-index.json.gz`) via fetch plus pako, without an IndexedDB cache. Both belong to the playground's third group, "Experimentelle Forschungsdaten" (#194), which collects curated datasets from external research projects; the entry convention is in that issue. The pattern is documented as a template in [DESIGN.md §Playground TEI-Analysis Module Pattern](DESIGN.md#playground-tei-analysis-module-pattern).
