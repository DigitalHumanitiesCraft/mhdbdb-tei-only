...
**Where the fold may be used:** the modern German and English descriptors of the authority files. Three call sites today, and `grep -rn "matchesFolded\|foldDiacritics" playground/js assets/js` is the way to check the list is still complete: `SearchPatterns.multiFieldNormalized` (the concept, genre, name and work explorers), `resolveQuery` in `concept-distribution.js`, and `findAlternativeMatch` in `concept-explorer.js`. The third is the one that is easy to forget: it decides the "auch: ..." hint rather than the hit list, and it had to learn the fold in the same move, or a folded hit would be listed with no explanation of why it matched. **Where it may not:** anything that compares Middle High German attestations. `normalizeMHG` is the contract there, it is parity-tested against Python, and a fold applied to corpus forms would silently widen matches that §C's three-stage lemma resolution deliberately keeps narrow.

Measured on 2026-09-11 over all four authority sets (567 concepts, 615 genres, 90 names, 584 works). **446 entries carry ä/ö/ü**, counted over exactly the fields these call sites compare and no others: concepts `termDE`/`termEN`/`altDE`/`altEN` (175), genres `termDE`/`termEN` (98), names `termDE`/`termEN` (23), works `title`/`author`/`sigle` (150). Capitals count, because the fold lowercases first (Ämter, Ölproduktion and 10 more). Two traps this number has already fallen into, both worth repeating before anyone recounts it: over `termDE`/`termEN`/`title` alone it is 391, and a probe that reads the field names off `items[0]` never sees the alternative terms at all, because the first concept has none. Genres carry 250 `altDE` that no caller reads, so they are correctly outside the count.

Result: **exactly one pair becomes equal under the fold that differs under `normalizeMHG`**, and it is `Vogel` / `Vögel` within `concept_14020000`. Both name the same concept, so no entry becomes indistinguishable from another.
