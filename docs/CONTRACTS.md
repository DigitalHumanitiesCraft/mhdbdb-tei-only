...
- **Full records** are the index records verbatim, plus `license`, with two transformations: `persons.works` is normalized from comma-string to a JSON array; texts are stripped of the heavy `words`/`lemmata`/`lineStarts`/`lineEnds` arrays (full token data stays in `data/corpus-index.json.gz` and `tei/`). Verbatim means additive index fields flow into the API automatically – e.g. lemma records carry `posAll[]` (every `<pos>` value; `pos` stays the first one) since Authority Index v1.6.0 (#161), and the curated fields `origin` (source language + attribution) plus `senses[].definition` / `senses[].comment` since v1.7.0. The curated fields are **present only where curated** (1 lemma today) – consumers must treat them as optional, never as a schema promise per record.
