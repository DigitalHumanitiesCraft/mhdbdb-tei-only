...
- **Verszählung pro Zählungsbereich (#138):** Die sichtbare Randnummerierung setzt bei jedem `<div>` wieder mit der 1 ein, das eine eigene **durchlaufende** Zählung bei `n="1"` beginnt. Kriterium sind zwei Bedingungen: die erste numerische `<l>` trägt `n="1"`, und die 1 kommt im `<div>` genau einmal vor. Betroffen sind **1.497 `<div>`s in 137 Texten** (897 `chapter`, 257 `song`, 177 `section`, 159 ohne `@type`, 7 `number`); zusätzliche Randnummern entstehen in 40 Texten, korpusweit 1.360. Größter Fall ist PZ (Parzival) mit 827 kapitelweise neu zählenden `<div>`s, gefolgt von FR3 (+141) und HUG (+39, Julias Ausgangsfall).

  Die zweite Bedingung ist der eigentliche Schutz: Texte mit **strophenlokaler** Zählung innerhalb eines `<div>` bekommen bewusst keinen Anker. NLA (Nibelungenlied Hs. A) etwa hat 38 untypisierte `<div>`s, in denen jede Strophe wieder bei 1 beginnt und die mangels `@type`/`@n` gar keine Überschrift rendern; ohne die Bedingung stünden dort 38 unmotivierte Randeinsen, also die #127-Regression über `<div>` statt über `<lg>`. Korpusweit verwirft die Bedingung 1.172 solcher `<div>`s. NBB ist von der Änderung strukturell gar nicht berührt, weil der Text keine `<div>`-Elemente enthält
