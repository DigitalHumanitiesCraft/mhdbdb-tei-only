...
- **Verszählung pro Zählungsbereich (#138):** Die sichtbare Randnummerierung setzt bei jedem `<div>` wieder mit der 1 ein, das eine eigene **durchlaufende** Zählung bei `n="1"` beginnt. Kriterium sind zwei Bedingungen: die erste numerische `<l>` trägt `n="1"`, und die 1 kommt im `<div>` genau einmal vor. Von 6.789 `<div>`s im Korpus erfüllen 2.661 die erste Bedingung, davon qualifizieren **1.473 in 137 Texten** (897 `chapter`, 252 `song`, 159 ohne `@type`, 137 `section`, 21 `parallel`, 7 `number`). Sichtbar werden dadurch **1.333 zusätzliche Randnummern in 49 Texten**. Größter Fall ist PZ (Parzival) mit +826, gefolgt von FR3 (+117), CHH (+53), TKR (+40) und HUG (+39, Julias Ausgangsfall). Alle Zahlen sind reproduzierbar: `python scripts/audit/count-verse-numbering-resets.py` baut die Render-Reihenfolge nach und vergleicht „mit Reset" gegen „ohne Reset". Stand 2026-07-30, nach dem Frauenlob-Umbau (#236): FR3 verliert 19 Anker, weil die eingehängte `div[@type="parallel"]` eine zweite `n="1"` in den Teilbaum ihrer `section` bringt, wodurch die sichtbare 1 vom Basiszeugen zum Parallelzeugen wandert. Ob die Zählregel verschachtelte Zeugen ausnehmen soll, ist als Frontend-Frage in #250 offen.

  Die zweite Bedingung ist der eigentliche Schutz: Texte mit **strophenlokaler** Zählung innerhalb eines `<div>` bekommen bewusst keinen Anker. Sie verwirft korpusweit 1.188 `<div>`s in 84 Texten und verhindert damit 1.026 unmotivierte Randeinsen. NLA (Nibelungenlied Hs. A) ist der Musterfall: 38 untypisierte `<div>`s, in denen jede Strophe wieder bei 1 beginnt und die mangels `@type`/`@n` gar keine Überschrift rendern; ohne die Bedingung bekäme der Text genau 38 zusätzliche Randeinsen, also die #127-Regression über `<div>` statt über `<lg>`. NBB ist von der Änderung strukturell gar nicht berührt, weil der Text keine `<div>`-Elemente enthält
