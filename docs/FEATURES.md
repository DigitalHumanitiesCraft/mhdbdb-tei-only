...
A conventional dictionary entry page (`woerterbuch.html`) for all 43,878 lemma pages.
...
- **Three perspectives on the same records** (Linda Beutel-Thurow, #59 comment of 2026-07-29 for the first two, #420 for the third). Every record carries three things: who is named, who does the naming, and with which term. Each of the three is an entry point:
  ...
  - *term*: pick a term, and the table lists the characters it names, with the attribution split out per character (narrator / character speech / self-naming) and two more columns saying in how many of those namings the term stood as a naming variant and as an epithet. This is the counter-direction to the first perspective and unreachable through it without walking every character of the work: in the Rolandslied `helt` falls 78 times on 22 characters, 61 of them from the narrator, while `hêrre` falls 222 times on 23 characters, 167 of them in character speech. Same material, opposite picture (Linda's own examples in #420)
- **Counting rule of the term perspective, and it is Linda's:** one row of the source is **one** naming. It counts once for the character named even when the term stands in several columns of that row. The two group columns count how many of those namings had the term in that group, so their sum can exceed the number of namings; in the Trojanerkrieg `got` names Jupiter 17 times, 17 as a naming variant and 1 of those additionally as an epithet. That overlap is rare: in the whole index exactly **three** records carry the same term in both groups (IW `der`, ROL `der`, TRO `got`; ENE none). Not to be confused with the 23 to 90 terms per work that stand in both groups across *different* records, where the two columns simply add up to the namings. The share column refers to the namings currently displayed, not to all of the term in the work; the reference is in the column header. That is the one deviation from Linda's spec, and it only concerns the speaker sub-filter, which her spec does not have: for the category tabs her `total_mentions` is the scope's total as well. The column does **not** reliably add up to 100, because the rounding happens per row: `helt` in the Rolandslied sums to 98 and `hêrre` to 95, and Linda's own reference file does the same for `helt`. Of 433 terms with at least two characters, 247 hit exactly 100; the range is 89 to 109
...
- Attribution visible in the module, rendered from `source` in the index rather than written out a second time, so a version bump moves it: as of 2026-09-07 Beutel-Thurow, L. (2026). Naming-analysis (v0.2.2-beta), DOI 10.5281/zenodo.22248457, CC BY-NC-SA 4.0. A build guard (`pruefe_zitation`) compares version and DOI against the `CITATION.cff` of the source state and fails on a mismatch, since the weekly rebuild would otherwise ship new data under an old citation
...
**How it works:**
- Scans all 43,878 lemmata for shared concept references (pre-built `sense.conceptIds` in the authority index, extracted at build time from `<sense>`/`<ptr target="concepts.xml#...">`)
...
- Lemmata as a single bundle (`api/lemmata/index.json`, 43,878 full records) instead of 43k individual files
