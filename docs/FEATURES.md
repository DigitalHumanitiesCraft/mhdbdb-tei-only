...
- Attribution visible in the module, rendered from `source` in the index rather than written out a second time, so a version bump moves it: as of 2026-08-14 Beutel-Thurow, L. (2026). Naming-analysis (v0.2.1-beta), DOI 10.5281/zenodo.21916576, CC BY-NC-SA 4.0. A build guard (`pruefe_zitation`) compares version and DOI against the `CITATION.cff` of the source state and fails on a mismatch, since the weekly rebuild would otherwise ship new data under an old citation
