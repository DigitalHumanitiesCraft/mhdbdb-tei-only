...
- The source notation stays visible. A leading `#` (Rolandslied, twice in the Trojanerkrieg) and square brackets (Trojanerkrieg) mark an instance that is not an acting character of the work: an indeterminate crowd (`#haiden`, `[MENGE]`) or a person merely quoted by the narrator (`#David`). The module explains this below the selection instead of cutting the notation off, and the grouping key that used to cut it is gone: it existed to merge five namers that had split into two entries each, and Linda unified those in the source on 2026-08-10 (`b7cc0585`). Measured against that state the key merges nothing (4 works, 33/29/48/71 namers, identical with and without the cut), and cutting would now be wrong, since `#David` and an acting David would become the same namer
