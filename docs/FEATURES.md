...
- Filter: Eigennamen ausblenden (NAM, Default an – 28 % der Hapaxe; eine explizit gewählte NAM-Facette schlägt den Filter, analog zu NUM), Zahlwörter ausblenden (Default an – greift nur bei reinem NUM, nicht bei Mehrfach-Wortarten wie `zwispeltic` ADJ/NUM; betrifft 72 der 119 NUM-Hapaxe. Anlass waren die drei Ziffern-Lemmata 42/46/49, die alphabetisch auf den Rängen 1 bis 3 standen, siehe #228. Eine explizit gewählte NUM-Facette schlägt den Filter, #196), Funktionswörter ausblenden (geteilte `FUNCTION_WORD_POS`-Menge aus word-frequency.js), Wortarten-Facette, Anfangsbuchstaben-Facette (auf `lemma.normalized`)
