...
- **Verszählung pro Zählungsbereich (#138):** Die sichtbare Randnummerierung setzt bei jedem `<div>`, das seine Zählung bei `n="1"` neu beginnt, wieder mit der 1 ein. Das betrifft **183 Texte mit 2.540 solchen `<div>`s** (1.269 `song`, 876 `chapter`, 178 `section`), nicht nur HUG; in 81 davon entstehen dadurch zusätzliche Randnummern, korpusweit 2.357. Größter Fall ist PZ (Parzival) mit 827 kapitelweise neu zählenden `<div>`s. Texte mit durchlaufender Zählung bleiben unverändert, weil dort nur der erste `<div>` bei `n="1"` beginnt. Texte, die pro **Strophe** zurücksetzen (NBB, `l/@n` 1..4 je `<lg>`), bleiben ebenfalls unverändert: der Reset greift ausschließlich auf `<div>`-Ebene, eine Marginal-1 in jeder Strophe wäre genau das unruhige Randbild, das #127 beseitigt hat
