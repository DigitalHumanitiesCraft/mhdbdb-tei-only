...
- **Two perspectives on the same records** (Linda Beutel-Thurow, #59 comment of 2026-07-29). Every record carries both who is named and who does the naming, and until 2026-08 only the first direction was selectable:
  ...
  - *naming character*: pick a namer, and the terms come grouped by the character named („which namings for whom does Iwein use in the Iwein?" is Linda's own example). The narrator is one of the selectable namers, and a character's self-namings count towards that character as namer
- Namers are grouped across the source notation. The namer column carries a leading `#`, underscores and shifting capitalization in the Rolandslied, which split five namers into two entries each (`#David`/`David`, `#Engel`/`Engel`, `#Cristen`/`#cristen`, `#Fürsten`/`#fürsten`, `#Karlinge`/`#karlinge`). Measured on 2026-08-09, the grouping key merges exactly those five and nothing else, and changes nothing in the other three works; the raw spellings stay readable in the title attribute. Deliberately in the view rather than in the index, because the notation is Linda's and is under query
...
- Data basis: `data/naming-index.json.gz` (~110 KB), built via `scripts/ingest/naming/01-fetch-and-build-index.py`, lazily loaded without an IndexedDB cache. One work-specific alias addition sits on our side in `scripts/ingest/naming/alias-overrides.json`: „Alexander" is the alias Paris goes under in the Trojanerkrieg, so the naming names him rather than describing him, and it counts as a proper name there. Linda's `lemma_normalization.json` normalizes across all works and would carry the alias into places where it is wrong (approved by Linda, #59 comment of 2026-07-28; affects exactly one record, TRO V. 20665)
