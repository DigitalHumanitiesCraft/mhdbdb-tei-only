...
- **Die zwei Herkunfts-Schichten sagten Unterschiedliches.** Konzept-Zeiger `{la, arc}`, `<etym type="borrowing">` nur `{arc}`, Bruecke war ausschliesslich der deutsche Prosasatz. Aufgeloest durch `<lang norm="la">` zusaetzlich: beide Schichten sind jetzt deckungsgleich, und die Regel dazu steht als solche in TEI-MODEL-AUTH-FILES. Welche Sprache Quelle und welche Vermittlung ist, sagt die Attributionsnotiz; maschinell ist die Liste ungeordnet, das haelt CONTRACTS §G.3 fest.
- **`@resp` war ungetypt und von keinem Gate abgedeckt.** Ein `contrib_03` waere durch beide Validierungsstufen, beide Audits und die CI gelaufen. Jetzt Pattern im Schema (negativ getestet: Tippfehler in `@norm` und `@resp` werden abgewiesen) plus `'resp'` in `audit-authority-files.py`. Der Cross-Ref-Wert 396 war deshalb kein Beweis: das Skript scannt nur `tei/`. Damit gatet die CI die **Form** des Werts, nicht die **Existenz** des Ziels: `audit-authority-files.py` ist ein manuelles Diagnose-Werkzeug und in keinem Workflow verdrahtet, ein `contrib_099` (es gibt 52 Einträge) validiert also grün. Bewusst so gelassen, weil das Skript 35 Bestands-Orphans meldet und als hartes Gate erst eine Baseline bräuchte.
- **`@norm` auf BCP-47 festgenagelt**, nicht „ISO 639-3 / BCP-47“. Fuer Aramaeisch identisch (`arc`), fuer Latein nicht (`la` gegen `lat`) – und Schicht A nutzt BCP-47.
- **Ausgelagert:** `@resp` wird in beiden Oberflaechen nicht angezeigt, weil `contributors.xml` bewusst nicht im Authority-Index liegt. Die Zurechenbarkeit endet damit auf der Datenebene. In TEI-MODEL-AUTH-FILES und CONTRACTS als Ist-Zustand dokumentiert, Umsetzung als eigenes Issue.
...
**Scorecard:** Die Datenzahlen der fünf geprüften Docs sind drift-frei (667 Korpusdateien, 8 Authority-Files, 43.879 Lemmata, 256.760 Formen / 234.243 Mappings, Index v4.2.0 / v1.7.0, alle gegen die Daten gemessen, nicht gegen andere Doku); Drift lag bei den **code-abgeleiteten** Zahlen und außerhalb des Reviers (#277 bis #279). Dreizehn Stellen in INDEX/ARCHITECTURE/DECISIONS/DESIGN korrigiert (neun davon in ARCHITECTURE), darunter fünf falsche Modulzahlen in zwei Dateien (2× 21 und 2× 22 für `playground/js/ui/`, Ist 25; 1× 10 für `ui/tei/`, Ist 13), zwei im Modulbaum und in der Router-Tabelle fehlende Werkzeuge (`hapax-legomena`, `verse-ending-profile`) und die Verwechslung von Varianten-Formen mit Varianten-Mappings. Spot-Checks ohne Abweichung: `lemmaRefMatchesId` gegen CONTRACTS §B.1, `isStage3Match`/`stage3Distance` gegen §C, `computeKeyness`/`logLikelihood` gegen die #114-Beschreibung in FEATURES (für Keyness gibt es keinen Contract, nur Prosa: genau deshalb #281); XPaths `etym[@type="morphological"]//seg[@type="component"]`, `sense/ptr[concepts.xml#]`, `idno[@type="handschriftencensus"]` gegen `build-authority-index.py`. Rebuild-Test: Suche, Build-Pipeline und Reader sind aus den Docs rekonstruierbar, die elf Playground-Analyse-Werkzeuge nicht (#281). Action Items: #276 bis #281.

**Der Hauptbefund ist das Gate, nicht die Zahlen.** `doc-count-audit.py` lief vor und nach den Korrekturen grün. Zwei der Fehler, die dieser Durchgang gefunden hat, wurden testweise wieder eingebaut und passierten `--check` mit Exit 0: `ARCHITECTURE.md` und `DECISIONS.md` stehen nicht in `DOC_TARGETS` (nur `CODE_DOC_TARGETS` erfasst sie, und das prüft drei Werkzeug-Counts), es gibt keinen Count für die UI-Modulzahl, und der Anker für `variants_normalized` kennt die Schreibweise „Varianten-Schlüssel" nicht. Die Lehre ist Handwerksregel 1 an einem Gate statt an einem Test: ein Audit, das jahrelang grün läuft, sagt nichts, solange niemand den Fehler einbaut, den es fangen soll.
...
**Zwei Zahlen, die verschieden bleiben müssen.** 256.760 Rohformen in `variants.xml` gegen 234.243 Mappings im Runtime-Dictionary, und 43.879 Lemmata heute gegen 43.754 vor dem #115-Backfill. Beide Paare sind mehrfach zu einer Zahl verschmolzen worden, in beide Richtungen. Korrigierte Stellen nennen jetzt den Unterschied mit, statt eine Zahl zu wählen: ARCHITECTURE §Data Layer führt Mappings und Formen nebeneinander, DESIGN markiert den Lexikonstand seines Messprotokolls, INDEX nennt in der #45-Chronikzeile keine erstarrte Lemma-Zahl mehr.

**Zwei Befunde außerhalb der Zahlen.** `docs/ARCHITECTURE.md` beschrieb den Storage-Abschnitt so, als seien alle vier Object Stores des Playgrounds in Gebrauch, und stellte die 24 Stunden des ungenutzten `authority_files`-Stores neben die 30 Tage aus ADR-004, als wäre es derselbe Mechanismus. Gemessen hat nur `tei_files` einen Schreiber; die Indexe liegen seit dem gemeinsamen `CorpusLoader` in `MHDBDBMainSite` (#280). Und die Router-Parameter-Tabelle kannte `mode=component` aus #239 nicht, obwohl der Router den Fall behandelt.
