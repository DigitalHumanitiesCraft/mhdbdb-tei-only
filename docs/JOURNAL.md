...
**Nachtrag nach den Reviews (28.07. abends):** Vier Review-Agenten plus die Bot-Reviews haben drei Blocker gefunden, die alle bestätigt und behoben sind. Zwei davon sind für künftige Sessions lehrreich:

1. **Der #224-Fix hätte den gemeldeten Fall gar nicht behoben.** `variants["boeses"]` existiert im Index, „böses" kehrt in Stufe 2 zurück und erreicht Stufe 3 nie; meine beiden Tests wären auch auf main grün gewesen. Der entscheidende Hinweis stand im Screenshot: dort fehlt `bœse` unter den Treffern, obwohl es ein Präfix ist. Die Eingabe trug ein **zerlegtes** Umlaut-ö (o + U+0308), verfehlte deshalb Stufe 1 und 2 und landete erst dadurch im Fallback. Der eigentliche Fix ist NFC-Komposition im Normalizer (Contract A, Schritt 0); der Authority-Index geht dadurch auf v1.6.2, weil drei Personen- und Werk-Datensätze mit zerlegtem ü bisher unauffindbar waren. Lehre: **wenn ein Test grün ist, prüfen, ob er auch auf main grün wäre.**
2. **Die #138-Zusicherung „NBB bleibt unverändert" war wertlos, weil NBB gar keine `<div>` hat.** Der echte Risikofall ist NLA mit 38 untypisierten divs und strophenlokaler Zählung; die erste Fassung hätte dort 38 unmotivierte Randeinsen erzeugt, also die #127-Regression über einen anderen Weg. Verschärfte Bedingung (die 1 muss im div genau einmal vorkommen) verwirft 1.172 solcher divs korpusweit und lässt HUG und PZ unberührt. Lehre: **eine Zusicherung, die strukturell trivial erfüllt ist, ist kein Schutz.**
3. Dazu ein CSS-Kaskadenfehler (`.back-to-top { display: flex }` schlug Tailwinds `.hidden`, der Button war permanent sichtbar) und drei Faktenfehler in meiner #196-Doku. Lehre zur Verifikation: **`classList.contains()` ist kein Sichtbarkeits-Check**, nötig ist die berechnete Anzeige.
